...
|---|---|---|
| **2.30.1** | 26 Sep 2026 | **Hotfix — the below-minimum prompt could not fire for the 12th, and the school years were never required.** A live applicant submitted 10th 85%, **12th 65%**, CGPA 8.6 with 8th and 9th left blank, and was never asked for a reason. Three separate holes: (1) the 12th was wired as `checkMark('12',0)` — "recorded, not scored" — so it could never be marked `fail`; (2) `updateBelowNote()` scanned only `['8','9','10','cgpa']`, so even a failing 12th would not have opened the panel; (3) **no academic field appeared in the required list at all**, so any of them could be left blank. Fixes: the 12th is now scored at 80% like every other year and published in the "What we look for" box; `updateBelowNote()` scans all five rows and names them from an `ACAD_LABELS` map instead of scraping label text (the old path would have read "12th Standard if applicable"); 8th, 9th and 10th are required whenever the academic block is showing, and current CGPA is required for everyone **except a 1st-year student**, who has no result yet. Anything typed into an academic field must now parse as a number — "NA" or "awaited" is rejected with a specific message, closing the obvious way around the check. `.has-error` styling and the clear-as-you-fix listeners were `.field`-only and blind to `.acad-row`; both now cover it, so a flagged academic field actually turns red. |
